...
//sélection des alternants correspondant au superviseur
    $base = new PDO('mysql:host=localhost;dbname=test;charset=utf8', 'root', '', array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
    $requete = $base->prepare('SELECT * FROM alternant WHERE ID_superviseur=? || ID2_superviseur=?');
    $requete->execute(array($_SESSION['dataalt']['ID_superviseur'], $_SESSION['dataalt']['ID_superviseur']));
...
if($_SESSION['ID_superviseur']!=NULL){
  $base = new PDO('mysql:host=localhost;dbname=test;charset=utf8', 'root', '', array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
  $requete_superviseur = $base->prepare('SELECT * FROM superviseur WHERE ID_superviseur=?');
  $requete_superviseur->execute(array($_SESSION['ID_superviseur']));
...
<br>
  ...
  <img class="image" src="<?php echo  htmlspecialchars($data_superviseur['file_url']);?>"/>
      ...
      <br>
      <br>
      <a href="Utilisateur.php" type="button" class="btn btn-outline-dark"><i class="fas fa-pencil-alt"></i>  Modifier mes informations</a>
      <br>
    </div>
    </div>
<?php
if($_SESSION['ID_superviseur']==$_SESSION['dataalt']['ID_superviseur'] || $_SESSION['ID_superviseur']==$_SESSION['dataalt']['ID2_superviseur']){
...
?>
  ...
  <div class="row">
    <div class="col-6">
      <?php 
      if($_SESSION['dataalt']['ID_alternant']!=NULL){
          $base = new PDO('mysql:host=localhost;dbname=test;charset=utf8', 'root', '', array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
          $requete_alternant = $base->prepare('SELECT * FROM alternant WHERE ID_alternant=?');
          $requete_alternant->execute(array($_SESSION['dataalt']['ID_alternant']));
          while($dataalternant = $requete_alternant->fetch())
          {
            $data_alternant=$dataalternant; 
          }
          $requete_alternant->closeCursor();
      }else{};
      ?>
      <br>
      <div class="text-dark bg-white border border-warning p-3 rounded">
        <img class="image" src="<?php echo  htmlspecialchars($data_alternant['file_url']);?>"/>
      <h5>ALTERNANT</h5>
      <br>
      <label>Prénom :</label>
      <?php 
...
if($_SESSION['ID_superviseur']==$_SESSION['dataalt']['ID_superviseur']){
...
}elseif($_SESSION['ID_superviseur']==$_SESSION['dataalt']['ID2_superviseur']){
...
<div class="row">
    <div class="col-6">
      <br>
      <a href="Page_de_réglementation_des_absences.php" type="button" class="btn btn-outline-dark">Réglementation des absences</a>
    </div>
  </div>
</div>
<br>
