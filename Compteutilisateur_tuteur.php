...
//sélection des alternants correspondant au tuteur
    $base = new PDO('mysql:host=localhost;dbname=test;charset=utf8', 'root', '', array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
    $requete = $base->prepare('SELECT * FROM alternant WHERE ID_tuteur=?');
    $requete->execute(array($_SESSION['dataalt']['ID_tuteur']));
...
if($_SESSION['ID_tuteur']!=NULL){
  $base = new PDO('mysql:host=localhost;dbname=test;charset=utf8', 'root', '', array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
  $requete_tuteur = $base->prepare('SELECT * FROM tuteur WHERE ID_tuteur=?');
  $requete_tuteur->execute(array($_SESSION['ID_tuteur']));
