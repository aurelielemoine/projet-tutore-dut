...
class Date{
	...
	function recup($annee){
		...
		while(date('Y-m-d',$date) < ($annee+1).'-11-01'){ 
							...
							<tr>';
								...
								foreach ($datesAM as $J=>$datesAMJ):
									...
									if(substr($tab[$p][0], 2)=="IUT"){
										...
										if ($periode >= $debut && $periode <= $fin)
										{#806D5A
										  echo ' style="background-color:#cee1b2" onclick="location.href=\'Periode_Entreprise_TRTE.php?date1='.$date1.'&&date2='.$date2.'\'" ';
										}
