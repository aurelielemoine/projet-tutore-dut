...
//sélection des alternants correspondant à l'enseignant
    $base = new PDO('mysql:host=localhost;dbname=test;charset=utf8', 'root', '', array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
    $requete = $base->prepare('SELECT * FROM alternant WHERE ID_enseignant=?');
    $requete->execute(array($_SESSION['dataalt']['ID_enseignant']));
...
<br>
    ...
    $requete = $base->prepare('SELECT * FROM enseignant WHERE ID_enseignant=?');
    $requete->execute(array($_SESSION['ID_enseignant']));
    while($data = $requete->fetch())
    {
      $data_enseignant=$data; 
    }
    $requete->closeCursor();
  ?>
